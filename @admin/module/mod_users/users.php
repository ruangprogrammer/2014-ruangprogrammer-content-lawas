...
switch($_GET['act']){
  ...
  default:
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             <td><a href=?module=users&act=edituser&id=$r[id]>Edit </a>";
    if ($r['level']=='admin'){
		echo "";
	}else{
		echo "<a href=$aksi?module=users&act=hapus&id=$r[id]> | Hapus </a></td></tr>";	
	}
      $no++;
    }
    echo "</table>";
  ...
  case "tambahuser":
    echo "<h2>Tambah User</h2>
          ...
          <tr><td>Level / Jabatan</td>   <td> : <input type=text name='level' size=20></td></tr>
  ...
  case "edituser":
    $edit=mysql_query("SELECT * FROM users WHERE id='$_GET[id]'");
    ...
    echo "<tr><td colspan=2>*) Apabila password tidak diubah, dikosongkan saja.<br />
                            **) Username tidak bisa diubah.<br />
							**) Untuk Mengubah Password klik <a style='text-decoration:underline;' href='?module=users&act=ubahpassword&id=$r[id]'>disini</a>.</td></tr>
  ...
  case "ubahpassword":
    ...
    echo "<h2>Edit Password $r[username]</h2>
          <form method=POST action=$aksi?module=users&act=ubahpassword>
          <input type=hidden name=id value='$r[id]'>
          <table>
          <tr><td>Password Lama</td>     <td> : <input type=password name='password'></td></tr>
          <tr><td>Password Baru</td>     <td> : <input type=password name='password_baru'></td></tr>
          <tr><td>Ulangi Password Baru</td>     <td> : <input type=password name='password_baru2'></td></tr>
          <tr><td colspan=2><input type=submit value=Update>
                            <input type=button value=Batal onclick=self.history.back()></td></tr>
          </table></form>";
    break;  
