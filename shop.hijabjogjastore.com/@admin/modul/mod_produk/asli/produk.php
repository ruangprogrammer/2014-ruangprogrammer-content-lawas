...
//Tampil Produk
    ...
    echo "<h2 class='hLine'>Produk</h2>
          ...
          <input type=button class='butt' value='Promo' onclick=\"window.location.href='?module=produk&act=promo';\">
          <input type=button class='butt' value='Terlaris' onclick=\"window.location.href='?module=produk&act=terlaris';\">
    ...
    while($r=mysql_fetch_array($tampil))
    {
      ...
      echo "<tr><td>$no.</td>
				...
				echo "</td>";
		        echo "<td>
		        <p><a href=?module=produk&act=editproduk&id=$r[id_produk]><img src='images/add-icon.gif' title='Edit Detail' /></a>
		        &nbsp;&nbsp;
		        <a href=$aksi?module=produk&act=hapus&id=$r[id_produk]><img src='images/hr.gif' title='Hapus' /></a>
				&nbsp;&nbsp;
				<a href=?module=subimages&id=$r[id_produk]><img src='images/images.png' title='images' /></a> <br />
		        </p>
				</td>
		        </tr>";
      $no++;
...
//Tambah Produk Disini
    ...
    echo "<h2 class='hLine'><a href='?module=produk'>Produk</a> &raquo; Tambah Produk</h2>
          ...
          <tr>
			...
			<select name="kategori" id="kategori">
			  <?php  
				include "koneksi.php";
				$result = mysql_query("SELECT * FROM kategori", $link_id);	
				echo "<option value=''>-- Pilih kategori --</option>";
				while($baris=mysql_fetch_array($result))
				{
					echo "<option value='$baris[id_kategori]'>$baris[nama]</option>";
				}
			  ?>
			</select>
	...
	echo   "<tr>
          ...
          $muncul=mysql_query("SELECT * FROM subkategori ORDER BY nama");
          if ($r['id_subkategorikategori']==0){
            echo "<option value=0 selected>- Pilih Sub Kategori -</option>";
          }   
          while($sk=mysql_fetch_array($muncul)){
            if ($r['id_subkategori']==$sk['id_subkategori']){
              echo "<option value=$sk[id_subkategori] selected>$sk[nama]</option>";
            }
            else{
              echo "<option value=$sk[id_subkategori]>$sk[nama]</option>";
            }
          }
    echo "</select></td></tr>";
