...
// Input
elseif ($module='sosmed' AND $act=='input'){

	if(empty($_POST['judul']))
	{
			echo"<script>window.alert('Ups! Judul masih kosong');
				window.location=('../../media.php?module=sosmed&act=tambahsosial')</script>";
	}
	else
	{	
...
// Update 
elseif ($module='sosmed' AND $act=='update')
{
	...
	if (empty($lokasi_file))
	{
		...
		mysql_query("UPDATE mod_sosial SET nama   = '$judul',
									   link   = '$url'  
								 WHERE id     = '$idsosial'");
		  header('location:../../media.php?module='.$module);
	...
	//jika ada foto yang diupload, tipe file harus jpeg atau png
		...
		{
			echo "<script>
			window.alert('Upload Gagal, Pastikan File yang di Upload bertipe *.JPG atau *.PNG');window.location=('../../media.php?module=sosmed&act=tambahsosial')
			</script>";
			unlink ($lokasi_file);
			exit();
		}
		else 
		{
			...
			$data=mysql_fetch_array(mysql_query("SELECT gambar FROM mod_sosial WHERE id='$idsosial'"));
			if ($data['gambar'] !='')
			{
			//hapus gambar dari database 
				mysql_query("DELETE gambar FROM mod_sosial WHERE id='$idsosial'");
			//hapus gambar dari folder 	
				unlink("../../../assets/images/sosmed/{$data[gambar]}");   
			}
