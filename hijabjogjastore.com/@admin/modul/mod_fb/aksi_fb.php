...
elseif ($module='fb' AND $act=='input'){
	...
	if(empty($_POST['judul']))
	{
			echo"<script>window.alert('Ups! Judul masih kosong');
				window.location=('../../media.php?module=fb&act=tambahsosial')</script>";
	}
	else
	{	
...
elseif ($module='fb' AND $act=='update')
{
	...
	if (empty($lokasi_file))
	{
		...
		mysql_query("UPDATE mod_fb SET nama   = '$judul',
									   link   = '$url'  
								 WHERE id     = '$idsosial'");
		echo "<script>window.alert('Data telah diupdate.');window.location=('../../media.php?module=fb&act=editsosial&id={$idsosial}');</script>";
	}
	//jika gambar diganti
	else 
	{
	//jika ada foto yang diupload, tipe file harus jpeg atau png
		...
		{
			echo "<script>
			window.alert('Upload Gagal, Pastikan File yang di Upload bertipe *.JPG atau *.PNG');window.location=('../../media.php?module=fb&act=tambahsosial')
			</script>";
			unlink ($lokasi_file);
			exit();
		}
		else 
		{
			...
			$data=mysql_fetch_array(mysql_query("SELECT gambar FROM mod_fb WHERE id='$idsosial'"));
			if ($data['gambar'] !='')
			{
			//hapus gambar dari database 
				mysql_query("DELETE gambar FROM mod_fb WHERE id='$idsosial'");
			//hapus gambar dari folder 	
				unlink("../../../joimg/{$data[gambar]}");   
			}
